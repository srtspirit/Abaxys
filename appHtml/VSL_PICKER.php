...
<div class="hidden" 
	ng-init="
		SESSION_DESCR='Sales Picking';
		getVSL_PICK();
	" >

<!--
		idVSL_ORHE=0;ABlstAlias('idVSL_ORHE','idVSL_ORHE','vsl_pickerChk','vsl_pick');
		getUsers();
...
<textarea class="hidden" ab-updSuccess="" >
	...
	$scope.setFormPickOff($scope.VSL_ORST_PICID);
	$scope.idVSL_ORHE=0;
...
?>
					...
					<td style="padding-top:10px;vertical-align:top;" >&nbsp;
						<a h2ref="#/VSL_ORDERS/VSL_ORDERS/Process:VSL_ORDERS,Session:VSL_ORDERS,tblName:vsl_orhe" >
						<button ab-label="STD_REFRESH" 
							ng-click="setFormPickOff(VSL_ORST_PICID);idVSL_ORHE=0;ABlstAlias('idVSL_ORHE','idVSL_ORHE','vsl_pickerChk','vsl_pick');"
						>Sales</button>
						<span class="hidden v2isible-sm">SM</span> 
						<span class="hidden v2isible-lg">LG</span> 
						<span class="hidden v2isible-md">MD</span> 
						<span class="hidden v2isible-xs">XS</span> 
						</a>
					</td>
					<td style="padding-top:10px;padding-left:200px;" >
		<div class="row well ab-spaceless {{orderSelected==0||orderSelected==varRow.VSL_ORST_PICID?'hidden':''}} " style="border-color:RoyalBlue;" >
			...
			<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 text-left " >
				...
				<td style="width:5%;" >
					...
					<input type="button" 
						...
						ng-click="setFormPickOff(varRow.VSL_ORST_PICID);"
					 	value="Cancel"
					/>
...
<input class="hidden" ng-if="allPicksSelected==true&&VSL_ORSI_REISS==0&&stpSelName!=''"  size=5 ng-model="VSL_ORSI_PROCE" ng-init="VSL_ORSI_PROCE=1"  />		
			...
			<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 small text-right"  >
				...
				<li class="bg-warning" 
				ng-if="vsl_orhe[0].IV_VSL_STEPS_VALID.indexOf(step.name)>-1 && step.shade!='hidden' && step.name > 'FF_PICK' && step.name < 'II_DELI' " 
...
?>
	...
	<div class="row bg-primary " ng-init="extraSort='';sortBy='VSL_ORST_PDATE';" >
		...
		<div class="col-lg-1 col-md-3 col-sm-3 col-xs-3 hidden-xs hidden-md  hidden-sm  ab-spaceless" ><?php echo setHeadField("VSL_ORST_PICID","5","Picking Id.","","","","bg-primary "); ?>
...
-->
	...
	<div class="row">
 		<div ng-if="pickCheck==1" class="col-lg-2 well text-center" style="position:fixed;z-index:1;" >
  		...
  		<div ng-if="pickCheck!=1 && rawResult.vsl_pick.length==0" class="col-lg-2 well text-center" style="position:fixed;z-index:1;" >
 			<span class="text-primary" >Nothing to pick</span>
 		</div>						

	</div>
		
		<div class="row {{orderSelected==0||orderSelected==varRow.VSL_ORST_PICID?'':'hidden'}} ab-odd ab-border ab-spaceless" id="rNum{{varRow.abIndex}}" 
			ng-repeat="varRow in  rawResult.vsl_pick | AB_noDoubles:'idVSL_ORHE,VSL_ORST_PICID' | AB_Sorted:extraSort+sortBy " 
		...
		>	
			...
			<div class="col-lg-1 col-md-4 col-sm-6 col-xs-12   " >
				<table style="width:100%;" >
					<tr>
						<td style="width:30%;" class="text-left visible-xs visible-md  visible-sm ">
							<?php echo setHeadField("VSL_ORST_PICID","5","Pick Id","","","","text-primary visible-xs visible-md  visible-sm "); ?> 
						</td>
...
<div class=" col-lg-12 col-md-12 col-sm-12 col-xs-12  ">
	...
	<div class="row text-primary text-left" ng-if="orderSelected==varRow.VSL_ORST_PICID">
		<!-- orderSelected==0||orderSelected==varRow.VSL_ORST_PICID -->
		...
		<div class="col-lg-1 col-md-4 col-sm-6 col-xs-12 ab-spaceless  hidden-xs hidden-md  hidden-sm  ab-strong" ><?php echo setHeadField("VSL_ORDE_ORLIN","10","Line","","","","text-primary"); ?></div>
		<div class="col-lg-1 col-md-4 col-sm-6 col-xs-12 ab-spaceless  hidden-xs hidden-md  hidden-sm  ab-strong" ><?php echo setHeadField("VSL_ORST_STPSQ","10","Seq.","","","","text-primary"); ?></div>
...
<!--END-->
	...
	<div 
		class="row hidden" 
		ng-repeat="rowDetail in  rawResult.vsl_pick | AB_noDoubles:'idVSL_ORST,idVSL_LSTR' | AB_Sorted:sortBy " 
		ng-if="rowDetail.VSL_ORST_PICID==varRow.VSL_ORST_PICID"
	 >
	...
	<div id="formPick" class="row  ab-border {{orderSelected>0?'':'hidden'}}" ng-init="vslFormPg=2"  >	
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12  ">
				...
				<?php  require_once "VSL_ORHECT_PICK.php"; ?>	
