	...
	<div class=" row " ab-formlist="order_list"   ab-rowset="{{$index}}"   ab-new="{{ x.idVPU_ORDE < 1?'1':'0' }}"
	 ...
	 >
		...
		<div  class="AC col-sm-1  {{x.trash==1?'text-danger':''}} "  >
			...
			<span class="small  {{x.trash==1?'text-danger':'text-primary'}} {{vslFormPg!=2?'':'invisible'}}" style="white-space:nowrap;" >
					...
					<div class="mygrid-wrapper-div " style="max-height:250px;" >
						...
						<tr class="" 
						 ...
						 >		
								...
								ng-click="
								x.VPU_ORDE_LSPEC=toggleSpecList(smaRows.idVIN_SSMA,x.VPU_ORDE_LSPEC);x.VPU_ORDE_LLINK=getSpecColorSet(x);
								" >
...
echo '<div class="AC col-sm-1" ><strong>';

// VPU_ORDE_OUNET
$grAttr = $xtmp->grAttrib;
$grAttr["class"] .= " {{ x.VPU_ORDE_QUACC!=0?'hidden':''}} ";
$laAttr = $xtmp->laAttrib;
$inAttr = $xtmp->inAttrib;
// $laAttr["class"] = "hidden";
$inAttr["size"] = "4";
...
$hardCode = <<<EOC
				...
				<input class="hidden" ng-model="x.VPU_ORDE_QUACC" />
				<ul class="nav ab-pointer  ab-spaceless " role="tablist"    >
				<li class="dropdown ab-spaceless"  >
					<span  class="text-primary btn-link" data-toggle="dropdown" style="white-space:nowrap;padding:0px;" >
						<span ng-if="x.VPU_ORDE_QUACC<1" >Select quote Account</span>
						<span 
						ng-repeat="quote in rawResult.vin_cust" 
						ng-if="x.VPU_ORDE_QUACC==quote.idVIN_CUST" >
						{{quote.VGB_CUST_BPNAM}}</span>
					
						<span class="caret" ></span>
					</span>
					<ul class="dropdown-menu ab-spaceless" ab-flst="" role="menu"  >
						<li class="" ng-repeat="quote in rawResult.vin_cust" ng-if="quote.VIN_CUST_SUPPL == VPU_ORHE_BTCUS && quote.VIN_CUST_ITMID == x.VPU_ORDE_ITMID&&quote.VIN_CUST_QUONU.trim()!=''" >
						<a class="small"  ng-click="x.VPU_ORDE_OUNET=quote.VIN_CUST_STDCP;x.VPU_ORDE_QUACC=quote.idVIN_CUST" >
						<span ng-if="x.VPU_ORDE_QUACC==quote.idVIN_CUST" > X</span>
						<span>{{quote.VGB_CUST_BPNAM}}</span>
						-
						<span>{{quote.VIN_CUST_STDCP}}</span>
						</a>
						</li>
						<li ng-if="x.VPU_ORDE_QUACC>0" ng-click="x.VPU_ORDE_OUNET=VIN_ITEM_STDCP;x.VPU_ORDE_QUACC=0" >
						Reset Quote
						</li>
...
$haardCode = <<<EOC
<tr ng-repeat="quote in rawResult.vin_cust" ng-if="quote.VIN_CUST_SUPPL == VPU_ORHE_BTCUS && quote.VIN_CUST_ITMID == x.VPU_ORDE_ITMID&&quote.VIN_CUST_QUONU.trim()!=''">

<td colspan=100 class=" text-right small"  ng-init="x.QuoteFound=1;x.VPU_ORDE_OUNET=(x.idVPU_ORDE<1?quote.VIN_CUST_SELLP:x.VPU_ORDE_OUNET)" >
<span class="text-primary">Customer</span> 
{{quote.VGB_CUST_BPNAM}}
<span class="text-primary"> SellP.</span>
 {{'$'+quote.VIN_CUST_SELLP}} 
 <span class="text-primary">Quote</span> 
{{quote.VIN_CUST_QUONU}}
</td>

</tr>

<tr ng-if="!x.QuoteFound"  >
<td colspan=100 class="text-primary text-right small" >No PriceFound</td>
</tr>
</table>
EOC;
...
<table>
	<tr exp-list="1" id="exp_{{x.idVPU_ORDE}}" class="collapse {{collaps!=1?'':'in'}}" >
		<td colspan=100 class="ab-spaceless" style="padding-top:4px;" >
			...
			<span class="hidden">
				<span ab-label="VPU_SUPP_BAORA_SHO" ></span>
...
$grAttr = $xtmp->grAttrib;
$laAttr = $xtmp->laAttrib;
$laAttr["ab-label"] = "VPU_SUPP_BAORA_SHO";
$grAttr["class"] .= " small ";
$inAttr = $xtmp->inAttrib;
$inAttr["class"] = "hidden";
...
<tr ng-if="x.VIN_ITEM_LOTCT > 0" id="LotQt{{y.idVPU_ORST}}" class="hidden  ab-spaceless ">
	...
	<td colspan=100 style="vertical-align:top;margin:0px;" id="LotQt{{x.idVPU_ORDE}}" class="small ab-spaceless " ng-app=""  >
		...
		<div class="row  ab-spaceless" ng-init="lotHeadBg='bg-danger ab-body-curtain '" style="color:white;"  >
		...
		<div   	ng-repeat="rowlotQt in vin_item_vin_lshe  | AB_noDoubles:'idVIN_ITEM' "
		 ...
		 <div  class="row {{lotQt.VIN_LSHE_SOLDO==0?'':'hidden'}}"
		 	ng-repeat="lotQt in rowlotQt.rowSet   | AB_noDoubles:'idVIN_LSHE'  | AB_sortReverse:'VIN_LSHE_DOMDA' "   
		 ...
		 >
			...
			<div class="col-sm-1 ab-border ab-spaceless text-right">
				...
				<span ng-repeat="lotAlo in lotAloRow.rowSet  | AB_noDoubles:'idVIN_LSLQ' " ng-if="lotAlo.VIN_LSLQ_LOTSQ == lotQt.idVIN_LSHE" >
			...
			>
				...
				<div class="ab-border ab-spaceless " style="min-width:25px;min-height:16px;" >
					...
					<span ng-repeat="lotRows in vpu_orheLSTR | AB_noDoubles:'idVPU_ORHE' " ng-if="lotRows.idVPU_ORHE==idVPU_ORHE" >
						...
						<span id="lotId" 
						ng-repeat="lotSel in lotRows.rowSet  | AB_noDoubles:'idVPU_LSTR' " 
	...
	<div id="invQ{{x.idVPU_ORDE}}" idtarget="1" class="hidden"  >
		...
		<div class="small" style="font-weight:700;" ng-repeat="invQrec in vin_inve | AB_noDoubles:'idVPU_ORDE' " ng-if="vin_inve.length>0 &&  invQrec.VIN_INVE_ITMID == x.VPU_ORDE_ITMID"  >
			...
			ng-if="invQ.idVPU_ORHE != idVPU_ORHE && invQ.VPU_ORST_STEPS > 'EE_SCED' && invQ.VPU_ORST_STEPS < 'JJ_INVO' " >
				...
				&nbsp;{{ invQ.VPU_ORHE_ORNUM  }}&nbsp; {{invQ.VPU_SUPP_BPNAM}}&nbsp;
