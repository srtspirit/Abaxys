...
<div 	class=" row " 
	ng-repeat="curRow in  rawResult.vpu_rece | AB_noDoubles:'idVPU_ORHE,VPU_ORST_PAKID'"
	ng-if="curRow.VPU_ORST_PAKID==orderSelected "
 >
	<input class="hidden" ng-model="idVPU_ORHE" ng-init="idVPU_ORHE=curRow.idVPU_ORHE" />
	<input class="hidden" ng-model="VPU_ORHE_ORSTP" ng-init="VPU_ORHE_ORSTP=curRow.VPU_ORHE_ORSTP" />
	<input class="hidden" ng-model="VPU_ORHE_BTCUS" ng-init="VPU_ORHE_BTCUS=curRow.VPU_ORHE_BTCUS" />
	<input class="hidden" ng-model="VPU_ORHE_ODATE" ng-init="VPU_ORHE_ODATE=curRow.VPU_ORHE_ODATE" />
	<input class="hidden" ng-model="VPU_ORHE_ORNUM" ng-init="VPU_ORHE_ORNUM=curRow.VPU_ORHE_ORNUM" />

	
	<input class="hidden" ng-model="idVPU_ORSI" ng-init="idVPU_ORSI=curRow.idVPU_ORSI" />
	<input class="hidden" ng-model="VPU_ORSI_USLNP" ng-init="VPU_ORSI_USLNP='<?php echo $currUsr['userCode']; ?>'" />
	...
	<div class=" row " ab-formlist="order_list"   ab-rowset="{{$index}}"   ab-new="{{ x.idVPU_ORDE < 1?'1':'0' }}"
		ORDE-repeat="1"
		role="presentation" 
		ng-repeat="x in rawResult.vpu_rece | AB_noDoubles:'idVPU_ORDE,VPU_ORST_PAKID' | AB_Sorted:'VPU_ORDE_ORLIN' " 
		
		ng-if="x.idVPU_ORDE != 0 && x.VPU_ORST_PAKID==orderSelected"
		ceelass="{{x.VPU_ORDE_ORLIN > 0?'':'hidden'}}"
	 >
		<form   ab-view="vpu_orde" ab-main="vpu_orde" ab-context="0" ng-init="x.showLots=1" >
			<input class="hidden" ng-model="x.idVPU_ORDE" />
			<input class="hidden" ng-model="x.idVPU_ORHE" />
			<input class="hidden" ng-model="x.VPU_ORDE_ORNUM" />
			<input class="hidden" ng-model="x.VPU_ORDE_ORLIN" />
...
echo "<table><tr><td class=' visible-xs text-primary ab-strong '>Line&nbsp;</td><td>{{ x.VPU_ORDE_ORLIN }} </td></tr></table>";
...
x.VPU_ORDE_OUNET = abSessionResponse.VIN_ITEM_LISTP;
		...
		<td class=" ab-spaceless text-top" >	
			<div ng-click="ACVarDivToggle(x.idVPU_ORDE);" class="ab-pointer" >
				...
				<td style="white-space:nowrap;" class="ab-strong" >
					<span class="caret text-primary"></span><span class="{{x.VPU_ORDE_OTEXT.trim().length>0?'text-danger':''}} small" ng-click="" >...</span>
				</td>
				</tr>
				</table>
...
<div  class="AC col-lg-6 " >
	...
	<table  class=" ab-spaceless " style="width:100%;vertical-align:top;">
		...
		<tr  ab-formlist="orstep_list"  
			...
			ng-repeat="y in rawResult.vpu_rece   | AB_noDoubles:'idVPU_ORDE,idVPU_ORST'  "
			...
			<span class="{{stpSelName!=''?'':'hidden'}}">
				<span ng-if="stepRetract!=true" >
					<span class="ab-pointer" 
						ng-if="y.VPU_ORST_STEPS<stpSelName || y.VPU_ORST_STEPS==stpSelName"  
						ng-click="toggleAllocStepSel(y.idVPU_ORST);countCheckBox('xlinesel');" 
					>
						<input type="checkbox"	cert="0" xlinesel="{{x.VPU_ORDE_ORLIN}}-{{idVPU_ORST}}" class="glyph2icon glyphic2on-unchecked" style="font-size:medium;"
							ng-if="(',' + DOC_ORST + ',').indexOf(','+y.idVPU_ORST+',')==-1" />
							
						<input  type="checkbox"	checked cert="0" xlinesel="{{x.VPU_ORDE_ORLIN}}-{{idVPU_ORST}}" class="glyp2hicon glyph2icon-expand " style="font-size:medium;"
							ng-if="(',' + DOC_ORST + ',').indexOf(','+y.idVPU_ORST+',')>-1" />
					</span>	
				</span>
				<span ng-if="stepRetract==true" >
					...
					<span class="ab-pointer" 
						ng-if="y.VPU_ORST_STEPS>stpSelName && stpSelName!=''"  
						ng-click="toggleAllocStepSel(y.idVPU_ORST);countCheckBox('xlinesel');" 
					>
						<input type="checkbox" cert="0" xlinesel="{{x.VPU_ORDE_ORLIN}}-{{idVPU_ORST}}" class="glyph2icon glyphic2on-unchecked" style="font-size:medium;"
							ng-if="(',' + DOC_ORST + ',').indexOf(','+y.idVPU_ORST+',')==-1" />
...
$stepCode .= $xtmp->currHtml;
$stepCode .= '</td><td style="min-width:160px;width:160px;" >';
$stepCode .= '<span id="VPU_ORST_BACKORMESS{{ y.idVPU_ORST }}" class="text-danger hidden">Back Order:&nbsp;</span>';
$stepCode .= '<input class="ab-borderless ab-strong" readonly id="VPU_ORST_BACKORDQT{{ y.idVPU_ORST }}" ng-model="y.VPU_ORST_BACKORDQT" size=5 />';
...
<tr ng-if="x.VIN_ITEM_LOTCT > 0" id="LotQt{{y.idVPU_ORST}}" class="hidden  ab-spaceless ">
	...
	<td colspan=100 style="vertical-align:top;margin:0px;" id="LotQt{{x.idVPU_ORDE}}" class="small ab-spaceless " ng-app=""  >
		...
		<div   	ng-repeat="rowlotQt in vin_item_vin_lshe  | AB_noDoubles:'idVIN_ITEM' "
		 ...
		 >
			...
			<div ng-if="x.VPU_ORDE_LLINK<1" class="col-sm-4 col-xs-4 col-md-2 col-lg-2  ab-spaceless" ng-init="x.specCurrent=''">			
			...
			<div ng-if="x.VPU_ORDE_LLINK>0" class="col-sm-4 col-xs-4 col-md-2 col-lg-2  ab-spaceless" ng-init="x.specCurrent=x.VPU_ORDE_LLINK" >
			...
			<td  class="text-right" style="width:50%">
				...
				<table>
					...
					<tr>
						...
						<td >
							<input  type="checkbox"	 cert="1" xlinesel="{{x.VPU_ORDE_ORLIN}}-{{idVPU_ORST}}" style="font-size:medium;"
							ng-if="x.VPU_ORDE_CFCAT!='0'" 
						...
						<td >
							...
							<li class="dropdown ab-spaceless"  >
								<span data-toggle="dropdown" class="text-primary btn-link" style="white-space:nowrap;padding:0px;" >
									<input ng-if="x.VPU_ORDE_CFCAT=='0'" readonly type="text" size=10 class="small ab-pointer ab-borderless" placeholder="No Certs" />
									<input ng-if="x.VPU_ORDE_CFCAT=='1'" readonly type="text" size=10 class="small ab-pointer ab-borderless" value="Cert of C" />
									<input ng-if="x.VPU_ORDE_CFCAT=='2'" readonly type="text" size=10 class="small ab-pointer ab-borderless" value="Cert of A" />
									<input ng-if="x.VPU_ORDE_CFCAT=='3'" readonly type="text" size=10 class="small ab-pointer ab-borderless" value="Cert of C & A"/>
								...
								<ul class="dropdown-menu ab-spaceless hidden" ab-flst="" role="menu"  >
						 		...
						 		<td class="text-center small ab-strong"  ng-if="x.VIN_ITEM_LOTCT > 0">
					        			...
					        			<input type="text" readonly  ng-if="x.VPU_ORDE_LSPEC.split(',').length -1>0" class=" text-primary ab-borderless   small"  
					        				size=30 
					        				value=" {{x.VPU_ORDE_LSPEC.split(',').length -1}} spec(s) to provide with delivery" 
					        			/>

								        <input type="text"  readonly ng-if="x.VPU_ORDE_LSPEC.split(',').length -1<1" class=" text-primary ab-borderless   small"  
		 ...
		 <div title="Select Specs "  ng-if="x.VIN_ITEM_LOTCT > 0">
			...
			<div  class="small" >
				<div class="" >
					...
					<div class="" style="max-height:250px;"  ng-if="x.VPU_ORDE_LSPEC.split(',').length -1>0" >
						...
						<tr class="" 
							...
							neg-repeat="smaRows in sma.rowSet | AB_noDoubles:'idVIN_SSMA'  "
							ng-if="(','+x.VPU_ORDE_LSPEC).indexOf(','+smaRows.idVIN_SSMA+',')>-1"
						 >		
...
<div class="col-sm-12" >
	...
	<div id="invQ{{x.idVPU_ORDE}}" idtarget="1" class="hidden"  >
		...
		<div class="small" style="font-weight:700;" ng-repeat="invQrec in vin_inve | AB_noDoubles:'idVPU_ORDE' " ng-if="vin_inve.length>0 &&  invQrec.VIN_INVE_ITMID == x.VPU_ORDE_ITMID"  >
			...
			<div class="row" ng-repeat="invQ in vin_inve | AB_noDoubles:'idVPU_ORDE,idVPU_ORST'  | AB_Sorted:'VPU_ORHE_ORNUM,VPU_ORDE_ORLIN' "  
			neg-repeat="invQ in invQrec.rowSet | AB_noDoubles:'idVPU_ORDE,idVPU_ORST'  | AB_Sorted:'VPU_ORHE_ORNUM,VPU_ORDE_ORLIN' " 
