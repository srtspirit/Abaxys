...
//
	...
	function updateInvoice($ePost)
	{
		...
		if(count($delID)>0 && count($recSet)>0)
		{
			$occ = 0;
			while ($occ < count($delID))
			{
				$dtaSet["TOTSALES"] = 0;
				$dtaSet["EXPCHARGED"] = 0;
				$dtaSet["EXPBORNED"] = 0;
				...
				while ($wocc < count($recSet))
				{
					if ( $recSet[$wocc]["VSL_ORST_DELID"] == $delID[$occ] )
					{
						...
						if ($recSet[$wocc]["VSL_ORDE_OLTYP"] == "STD")
						{
							...
							$dtaSet["AVGCOST"] += ($recSet[$wocc]["VIN_ITEM_AVGCP"] * $recSet[$wocc]["VSL_ORST_ORDQT"] ); //E
							$dtaSet["TAXES"] = $this->taxCalculate($totAmt,$dtaSet["TAXES"]);
							
							
							// Update vsl_orde if average cost changed
							if ($recSet[$wocc]["VIN_ITEM_AVGCP"] != $recSet[$wocc]["VSL_ORDE_COSTP"])
							{
								$recSet[$wocc]["PROCESS"] = $ePost["PROCESS"];
								$recSet[$wocc]["SESSION"] = $ePost["SESSION"];
								
								$recSet[$wocc]["VSL_ORDE_COSTP"] = $recSet[$wocc]["VIN_ITEM_AVGCP"];
	...
	function dbFindMatch($dtaobj)
	{
		...
		$wTbls->postingSet = $wTblsORHE;
 		...
 		while ($occ < count($pSet))
 		{
 			...
 			$pSetResult[$occ]["TOTSALES"] = $pSet[$occ]["dtaSet"]["TOTSALES"];
