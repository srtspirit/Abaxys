...
//		{
//			$out .= "Object $obj belongs to class " . get_class($GLOBALS[$obj]);
//			$out .= " a subclass of $class\n";
//		} 
//		else 
//		{
//			$out .= "Object $obj does not belong to a subclass of $class\n";
//		}
//		return $out;
//	}
...
if ($inPost['METHOD'] == "REQINFO")
{
	...
	$xout = array();
	$test = new dbMaster($tbl,$schema);
	$xout[$tbl] = array();
 	$xout[$tbl] = dspTblInfo($test);
 	$refXout = $test->tblCtrts->dta;
 	$occ = 0;
 	while ($occ < count($refXout))
 	{
 		...
 		if (!$xout[$nTable])
 		{
		 	$test = new dbMaster($nTable,$schema);
		 	$xout[$nTable] = dspTblInfo($test);
		}
...
if ($inPost['METHOD'] == "REQSING" || $inPost['METHOD'] == "REQFULL")
{
	...
	if ($inPost["MATCH"] == "1")
	{
		...
		$test->dbFindMatch($inPost);
	}
	else
	{
		$test->dbFindFrom($inPost);
	}
	
	$test->methods = $hmethods;

	echo json_encode(array('posts' => $test));
	exit;
}
...
function dspTblInfo($test)
{
//[=DOC=]Sets Table schema Post information 

	$tblParam['tblName'] = $test->tblInfo->tblName;
	$tblParam['tblDesc'] = $test->tblInfo->tblComment;
	$tblParam['tblFields'] = $test->tblFlds;
	$tblParam['tblPrimary'] = $test->tblPrimary;
	$tblParam['tblUnique'] = $test->tblUnique;
	$tblParam['tblConstraints'] = $test->tblCtrts;
