	...
	$scope.ACVarFilterInit = function(colName,colVal)
	{
		...
		var occ = 0;
		while (occ < $scope["rawResult"]["vin_supp"].length)
		{	
			if ( !$scope["rawResult"]["vin_supp"][occ]["rowRowLog"] || !colName)
			{
				$scope["rawResult"]["vin_supp"][occ]["rowRowLog"] = new Object();
				$scope["rawResult"]["vin_supp"][occ]["rowRowLog"]["COLS"] = new Object();
				$scope["rawResult"]["vin_supp"][occ]["rowRowLog"]["hidden"] = "  ";
			}			
			...
			$scope["rawResult"]["vin_supp"][occ]["rowRowLog"]["COLS"][colName] = colVal;
			$scope["rawResult"]["vin_supp"][occ]["rowRowLog"]["hidden"] = " Ok ";
			
			var dta = $scope["rawResult"]["vin_supp"][occ]["rowRowLog"]["COLS"]
			
			// $scope["rawResult"]["vin_supp"][occ]["rowRowLog"]["hidden"] += "[" + colName + ":" + colVal  + "]";
			
			for (var i in dta)
			{
				if (dta[i].indexOf("'" + encodeURI($scope["rawResult"]["vin_supp"][occ][i]) + "'")<0)
				{
					$scope["rawResult"]["vin_supp"][occ]["rowRowLog"]["hidden"] = "hidden";// "Not" + encodeURI($scope["rawResult"]["vin_supp"][occ][i]) + "in" +  dta[i];
				}
		  	}
